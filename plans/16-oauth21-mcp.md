...
| Migrations run `0000`–`0008` (verified by listing `lib/db/migrations/`) | `0008_mcp_tokens.sql` is the newest | **Plan 14 claims `0009` and Plan 15 claims `0009`-or-`0010`.** This plan takes whatever `drizzle-kit` generates next — see §7 risk 5. A hand-written migration missing its `meta/` journal entry was a real bug found during Plan 13; verify the journal entry lands. |
...
| 5 | **Migration number collision with Plans 14 and 15**, both of which claim `0009`. | Whoever lands last takes the next free number. `drizzle-kit` derives it from the existing folder, so this resolves itself **provided the migration is generated, not hand-numbered** — and a hand-written migration missing its `meta/` journal entry was a real Plan 13 bug. Check `lib/db/migrations/` immediately before generating. |
...
- **Admin powers over MCP by any credential.** Constraint 3, carried over from Plan 13
  unchanged.
- **Shared agents over MCP.** Plan 15 deliberately leaves shared agents invisible to MCP; this
  plan changes nothing about that, so an OAuth-authenticated Desktop sees exactly what a PAT
  sees — the user's own agents.
