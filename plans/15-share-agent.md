...
| Migrations run `0000`–`0008`; `drizzle-kit` generates the filenames | `lib/db/migrations/` | This plan adds the next one — **but Plan 14 also claims `0009`**; see §7 risk 5. |
...
| 5 | **Migration number collision with Plan 14**, which also claims `0009`. | Whichever lands second takes `0010`. `drizzle-kit` generates the number from the existing folder, so this resolves itself **provided the second plan regenerates rather than hand-numbering** — and a hand-written migration missing its journal entry was already a real Plan 13 bug. Check `lib/db/migrations/` before generating. |
