...
| 5.3 | **✅ VERIFIED 2026-07-31, live against real Google endpoints (browser-driven by the user; `myagent.db` inspected read-only by the assistant to confirm the `oauth_account` row).** Google button hidden-when-unconfigured/shown-when-configured; auto-link path end to end (§3.7) — signed in with Google using the account matching the existing admin password account: logged into that account, no second account created, `oauth_account` row written with `provider=google`, correct `user_id`, `provider_email` matching; sign-out then sign-in-with-Google again → logged straight in via the existing link (Outcome 1), no duplicate row; login-mode with a **different** Google identity that has no matching account → refused cleanly (Outcome 3 guard: login mode never creates an account); brand-new signup via Google with a real invite code (generated through the real `/settings` admin UI) → account created, code consumed; password login still works unchanged. All confirmed working as designed. |
| 5.4 | **⬜ Deferred, not yet run.** Set `SESSION_TTL_SECONDS=120`, restart, log in, wait, confirm the session expires and the `?next=` round trip returns to the right page; then unset it and confirm 7 days returns. Confirm an invalid value refuses to boot |
