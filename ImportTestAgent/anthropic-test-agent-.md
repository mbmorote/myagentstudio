...
description: |
  Test for import on the workbench -  Compliance, safety, and governance overseer for Anthropic-facing workflows. Use proactively whenever a task involves Anthropic APIs, agent configuration, tool wiring, or production deployments, to enforce security, safety, and policy guardrails before changes are applied.
model: opus
effort: max
tools:
  - Read
