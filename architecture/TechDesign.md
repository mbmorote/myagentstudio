...
| 18 | `ConfigDef`/`SectionDef` platform-scoping (a catalog per platform, not one global catalog) | Schema | `lib/db/schema.ts` (`configDef.platform`/`sectionDef.platform`, composite unique on `(platform, key)`); migration `0005_catalog_platform_scoping.sql` | 🔶 Partial — schema + bootstrap-only seed (`lib/db/seed.ts`, `onConflictDoNothing`) built 2026-08-07; catalog rows are now DB-owned and admin-editable in principle, but `lib/blueprint/rules.ts`/`lib/blueprint/prompt.ts` and every consumer still read only the 'claude' rows implicitly — no admin UI, no platform-aware read path yet | Plan 01 review, 2026-07-26; schema landed 2026-08-07 |
...
| 18 | ~~`ConfigDef` platform-scoping~~ — **schema half done 2026-08-07**: `configDef`/`sectionDef` now carry `platform` + a composite `(platform, key)` unique index, and the seed is bootstrap-only (won't clobber a DB-edited row). Still open: an admin UI to actually edit/add catalog rows, and making `lib/blueprint/rules.ts`/`lib/blueprint/prompt.ts` platform-aware instead of implicitly Claude-only | An admin catalog UI is built, and/or a second platform's import/create support is actually being built |
